...
with open('anf_interactive.sh', 'r', encoding='utf-8') as f:
    content = f.read()

# Find the start of the show_config function
start_marker = "show_config() {"
start_pos = content.find(start_marker)

if start_pos == -1:
    print("Could not find show_config function")
    sys.exit(1)
...
if pos >= len(content):
    print("Could not find end of show_config function")
    sys.exit(1)

# New function content
new_function = '''show_config() {
...
new_content = content[:start_pos] + new_function + content[pos+1:]

# Write the new content
with open('anf_interactive_updated.sh', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Updated show_config function and saved to anf_interactive_updated.sh")
print(f"Replaced {pos - start_pos + 1} characters with {len(new_function)} characters")